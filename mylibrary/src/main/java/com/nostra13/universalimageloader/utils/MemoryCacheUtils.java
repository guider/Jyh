...
public final class MemoryCacheUtils {
	...
	public static String generateKey(String imageUri, ImageSize targetSize) {
		...
		return new StringBuilder(generateKey(imageUri))
				.append(URI_AND_SIZE_SEPARATOR).append(targetSize.getWidth())
	...
	public static String generateKey(String imageUri) {
		...
		if (host != null && host.endsWith("clouddn.com")) {// 七牛云存储做特殊处理
			int index = imageUri.indexOf('&');
			if (index > 0) {
				return imageUri.substring(0, index);
			}
		}
